...
-(BOOL)filmInfoCell:(FilmInfoCell *)filmInfoCell didUpdateLayoutWithHeight:(CGFloat)newHeight
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (film.discount_value.intValue == 0) {
            targetSection = SECTION_FILM_INFO;
        }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    else if(section == SECTION_FILM_INFO)
    {
        return 2;
    }
...
-(void)initHandleSwipeGestureRecognizer
{
    ...
    [gestureLeft setCancelsTouchesInView:NO];
...
- (void)scrollTableViewToTop
{
    ...
    if (isDislayingTempTable) {
        ...
        numberRowOfSection = [tempTable numberOfRowsInSection:targetSection];
        if (numberRowOfSection == 0) {
            return;
        }
...
//        LOG_123PHIM(@"in stack: %@", ((Film*)[filmStack lastObject]).film_name);
...
//        NSLog(@"-----%@", dictionary.curDictionary.description);
...
- (IBAction)showSessionTimeClick:(id)sender {
    ...
    {
        NSString *fileNameLoad = @"FimCinemaViewController";
        if ([[UIScreen mainScreen] bounds].size.height == 568)
        {
            fileNameLoad = @"FimCinemaViewController_7";
        }
        FilmCinemaViewController *cinemaFilmController = [[FilmCinemaViewController alloc] initWithNibName:fileNameLoad bundle:[NSBundle mainBundle]];
